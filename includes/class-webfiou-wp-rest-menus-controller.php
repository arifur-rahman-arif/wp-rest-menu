...
class WP_REST_Menus_Controller extends WP_REST_Controller {
    ...
    public function get_location_menu_items(\WP_REST_Request$request) {
        ...
        if ($request->get_param('slug')) {
            if ($menu_items = wp_get_nav_menu_items($request->get_param('slug'))) {
                $menu = $this->get_item_fields($request, $menu_items);
            }
        }
        ...
        $organized_menu = $this->organize_menu_items($menu_data);

        return rest_ensure_response($organized_menu);
    }

    /**
     * Organize the menu items in parent and child order
     * @param $menu_data
     */
    public function organize_menu_items(array $menu_data): array{
        $organized_menu = [];

        if (!is_array($menu_data)) {
            throw new Exception('Menu items must be an array');
        }

        foreach ($menu_data as $key => $menu) {
            // If a item parent is already present in the #organized_menu variable than we will
            // insert the current menu inside the parent menu
            if (array_key_exists($menu->menu_item_parent, $organized_menu)) {
                if (!$organized_menu[$menu->menu_item_parent]->children) {
                    $organized_menu[$menu->menu_item_parent]->children = [];
                }

                array_push($organized_menu[$menu->menu_item_parent]->children, $menu);
            } else {
                $organized_menu[$menu->ID] = $menu;
            }
        }
